...
- Name: checkout.spec.js >> complete checkout flow
...
```ts
  1  | export default class InventoryPage {
  2  |   constructor(page) {
  3  |     this.page = page;
  4  | 
  5  |     this.inventoryList = page.locator('.inventory_list'); // fallback
  6  |     this.addToCartBtn = page.getByRole('button', { name: /add to cart/i });
  7  |     this.cartIcon = page.getByRole('link', { name: /shopping cart/i });
  8  |   }
  9  | 
  10 |   async addFirstItemToCart() {
  11 |     await this.addToCartBtn.first().click();
  12 |   }
  13 | 
  14 |   async goToCart() {
> 15 |     await this.cartIcon.click();
     |                         ^ Error: locator.click: Test timeout of 60000ms exceeded.
  16 |   }
  17 | }
```